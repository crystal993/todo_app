...
db.collection("counter").findOne(
    ...
    var posts =  { _id: 총게시물갯수 + 1, 작성자: request.user.id, 제목: request.body.title, 날짜: request.body.date  };
...
db.collection("post").findOne(
    ...
    function (error, result) {
    console.log(result);
    response.render("detail.ejs", { data: result, isAuthenticated:request.isAuthenticated() });
    }
);
});
...
db.collection("post").findOne(
    ...
    function (error, result) {
    console.log(result);
    response.render("edit.ejs", { data: result, isAuthenticated:request.isAuthenticated() });
    }
);
});
...
db.collection("post").updateOne(
    ...
    function (error, result) {
    console.log("수정완료");
    response.render("list.ejs", {isAuthenticated:request.isAuthenticated()});
    }
);
});
